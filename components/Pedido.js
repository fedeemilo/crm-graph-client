...
import { ACTUALIZAR_PEDIDO, ELIMINAR_PEDIDO } from "../gql/mutations";
...
const Pedido = ({ pedido }) => {
  ...
  const nombre = _.get(pedido, "cliente.nombre");
  const apellido = _.get(pedido, "cliente.apellido");
  const telefono = _.get(pedido, "cliente.telefono");
  const email = _.get(pedido, "cliente.email");
  
  const [estadoPedido, setEstadoPedido] = useState(estado);
  const [clase, setClase] = useState("");

  // Mutation para cambiar estado de un pedido
  ...
  return (
    ...
    >
      <div>
        ...
        <select
          className={`mt-2 appearance-none border text-white p-2 rounded leading-tight focus:outline-none uppercase text-xs font-bold 
                       ${
                         estadoPedido === "PENDIENTE"
                           ? "bg-yellow-600 border-yellow-600 focus:bg-yellow focus:border-yellow-500"
                           : ""
                       }
                       ${
                         estadoPedido === "COMPLETADO"
                           ? "bg-green-600 border-green-600 focus:bg-green focus:border-green-500"
                           : ""
                       }
                       ${
                         estadoPedido === "CANCELADO"
                           ? "bg-red-600 border-red-600 focus:bg-red focus:border-red-500"
                           : ""
                       }`}
          value={estadoPedido}
